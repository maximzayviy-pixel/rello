...
const SECRET = process.env.WEBAPP_SECRET || BOT_TOKEN;
...
export function verifyInitData(initData: string) {
  ...
  const data_check_string = Array.from(url.entries())
    ...
    .map(([k, v]) => `${k}=${v}`)
    .join('\n');
  const secret = require('crypto').createHash('sha256').update(SECRET).digest();
  const hmac = require('crypto').createHmac('sha256', secret).update(data_check_string).digest('hex');
