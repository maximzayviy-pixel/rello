...
function weekDateString(d = new Date()) {
  const week = startOfWeek(d, { weekStartsOn: 1 });
  return week.toISOString().slice(0, 10); // YYYY-MM-DD
}
...
export async function registerTap(tg_id: number, power = 1) {
  ...
  const weekStr = weekDateString();
  await db`
    insert into growth_scores (tg_id, week_start, growth)
    values (${tg_id}, ${weekStr}::date, ${power})
...
export async function topLeaderboard(limit = 50) {
  const weekStr = weekDateString();
  const weekly = await db`select tg_id, growth from growth_scores where week_start=${weekStr}::date order by growth desc limit ${limit}`;
...
export async function currentPool() {
  const weekStr = weekDateString();
  const pool = await db`select * from weekly_pools where week_start=${weekStr}::date`;
  return pool.rows[0] || { week_start: weekStr, total_xtr: 0, prize_xtr: 0, distributed: false };
}
