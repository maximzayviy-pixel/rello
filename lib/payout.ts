...
export async function distributeWeeklyPrize() {
  ...
  await db`
    ...
    values (${lastWeekStr}::date, 0, 0, false)
    on conflict (week_start) do nothing`;

  const pool = (await db`select * from weekly_pools where week_start=${lastWeekStr}::date`).rows[0];
  if (pool?.distributed) return { status: 'already-distributed' };

  const scores = (await db`select tg_id, growth from growth_scores where week_start=${lastWeekStr}::date order by growth desc limit 100`).rows;
